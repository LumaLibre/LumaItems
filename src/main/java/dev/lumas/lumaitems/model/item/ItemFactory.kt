...
class ItemFactory(
    ...
    fun <T : Any> addDataComponents(type: DataComponentType.Valued<T>, value: T): ItemFactory {
    ...
    fun createItem(): ItemStack {
        ...
        if (spoofEnchants) {
            for (enchant in vanillaEnchants) {
                ...
                combinedLore.add("<gray>${Util.formatEnchantKey(enchant.key.key.toString())}$postAppend")
            }
            hideEnchants = true
        }
        ...
        if (tier.isNotEmpty()) {
            val tierFormat = if (miniMessage) TIER_FORMAT else LEGACY_TIER_FORMAT
            combinedLore.addAll(tierFormat.map { it.format(tier) })
        }

        meta.displayName(name.colorcode(!miniMessage))
        meta.lore(combinedLore.colorcode(!miniMessage).map { it.colorIfAbsent(NamedTextColor.WHITE) })
