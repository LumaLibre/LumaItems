...
class BlobbleHammerItem : CustomItemFunctions() {
    ...
    companion object {
        ...
        ).mapValues { ItemStack(it.value.first).apply { addUnsafeEnchantment(Enchantment.UNBREAKING, 1) } to ParticleDisplay.of(
            Particle.DUST).withColor(Color.decode(it.value.second)) }
