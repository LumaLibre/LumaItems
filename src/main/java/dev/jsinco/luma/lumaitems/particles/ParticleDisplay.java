...
public class ParticleDisplay implements Cloneable {
    ...
    public static ParticleDisplay colored(@Nullable Location location, int r, int g, int b, float size) {
        return ParticleDisplay.of(Particle.DUST).withLocation(location).withColor(r, g, b, size);
    }
    ...
    public static ParticleDisplay colored(Location location, @Nonnull Color color, float size) {
        return ParticleDisplay.of(Particle.DUST).withLocation(location).withColor(color, size);
    }
    ...
    public static ParticleDisplay edit(@Nonnull ParticleDisplay display, @Nonnull ConfigurationSection config) {
        ...
        Optional<Particle> particle = particleName == null ? Optional.empty() : Optional.of(Particle.valueOf(particleName.toUpperCase()));
        particle.ifPresent(Particle -> display.particle = Particle);
    ...
    /**
     * Mixes the current color with the given color.
     * @param color the color to mix with.
     * @return the same particle display, but modified.
     */
    public ParticleDisplay mixWith(@NotNull Color color) {
    ...
    private void spawnRaw(Particle particle, Location loc, int count, Vector offset, Object data) {
        ...
        double extra = (this.particle == Particle.DUST) ? 1 : this.extra;
        if (players == null)
            if (ISFLAT)
    ...
    public static class RGBParticleColor implements ParticleData {
        ...
        public Vector offsetValues(ParticleDisplay display) {
            ...
            if (!ISFLAT || (display.particle == Particle.ENTITY_EFFECT
                    && display.particle.getDataType() == Void.class)) {
        ...
        public Object data(ParticleDisplay display) {
            ...
            if (display.particle == Particle.DUST) {
                return new Particle.DustOptions(
                        org.bukkit.Color.fromRGB(color.getRed(), color.getGreen(), color.getBlue()),
                        particleSize
                );
            } else if (display.particle == Particle.DUST_COLOR_TRANSITION) {
        ...
        public ParticleData transform(ParticleDisplay display) {
            if (display.particle == Particle.NOTE) {
